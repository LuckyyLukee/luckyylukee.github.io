...
function createCards() {
  ...
  for (let i = 0; i < symbols.length; i++) {
    ...
    card.addEventListener("click", (e) => { // Här lägger vi på en EventListener på varje kort så vi kan ta emot klick eventet, alltså vad som ska hända när man trycker på kortet
...
function updateHistory() {
  ...
  if (gameState.playerTurn == 0) {// Spelare 1
    gameState.history +=
      "<p> " +
...
function checkIfCardsMatch() {
  if (firstCardSymbol == secondCardSymbol) { //Detta körs om båda korten matchar varandra
    ...
    setTimeout(() => { // Efter en sekund så läggs klass "hide" på korten, då försvinner dom. Korten tas inte bort helt utan dom blir bara osynliga.
      firstCard.classList.add("hide");
      secondCard.classList.add("hide");
      checkIfGameEnded(); // Kolla sedan om det var dom sista korten så matchen kan ta slut.
    }, 1000);
  } else { // Detta körs om korten inte matchade varandra
...
function checkCardsLeft() {
  ...
  for (let i = 0; i < allCards.length; i++) { // Loopar igenom alla kort 
...
function checkIfGameEnded() {
  ...
  if (checkCardsLeft() == 0) { // Kolla om det är 0 kort kvar, om det är 0 kort kvar så har matchen tagit slut
...
function flipCard(card, symbol) {
  ...
  if (!gameState.currentlyFlipped && !card.currentTarget.classList.contains("hide")) { //If-sats som förhindrar spelaren att öppna flera kort medans 2 redan är vända, eller öppna ett kort som redan har blivit hittat.
    card = card.currentTarget;
    card.classList.add("flip"); //Lägger till styling-classen flip på kortet som tryckts på

    if (gameState.cardsFlipped == 0) { // Detta körs för första kortet
    ...
    } else if (card != firstCard) { // Detta körs för andra kortet som väljs, kollar så man inte tryckt på samma kort som första.
